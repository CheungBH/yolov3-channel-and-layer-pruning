...
def train():
    ...
    if "finetune" in opt.wdir:
        write_csv = True
    if "finetune" in opt.wdir or "distillation" in opt.wdir:
        wdir = opt.wdir + os.sep
    else:
        wdir = os.path.join('weights/sparse', opt.wdir) + os.sep  # weights dir
    ...
    if "finetune" in wdir:
        ...
        print(wdir)
        shutil.copy(cfg, wdir)
    elif "distillation" in wdir:
        tb_writer = SummaryWriter(os.path.join("runs", wdir[13:]))
        shutil.copy(cfg, wdir)
    else:
        tb_writer = SummaryWriter(os.path.join("runs", wdir[8:-1]))
    ...
    if write_csv:
        ...
        with open(os.path.join(csv_path, 'prune.csv'), 'a+') as f:
            ...
            if not exist:
                ...
                f_csv.writerow(title)
            info_list = [model_name, best_fitness, '', '']
    ...
    with open(results_file, "a+") as f:
        f.write("\n")
        f.write("{},{},".format(flops, params))
        res = res2list(best_result)
        for i in res:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data', type=str, default='data/fish/fish.data', help='*.data file path')
