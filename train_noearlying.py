...
def train():
    ...
    def adjust_learning_rate1(optimizer, gamma, epoch, iteration, epoch_size):
        """调整学习率进行warm up和学习率衰减
        """
        step_index = 0
        if epoch < 6:
            # 对开始的6个epoch进行warm up
            lr = 1e-6 + (config.hyp['lr0'] - 1e-6) * iteration / (epoch_size * 2)
        else:
            ...
            lr = config.hyp['lr0'] * (gamma ** (step_index))
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            if epoch < config.warm_up:
                ...
                lr = adjust_learning_rate1(optimizer, 0.1, epoch, ni, nb)
            imgs = imgs.to(device)
        ...
        if save:
            ...
            # torch.jit.save(model.state_dict(),'weigh.pt')
